...
public class UserGUI extends JDialog implements ActionListener {
	...
	public UserGUI(TableExam me, String index) {
		super(me, "다이어로그");
		this.me = me;
		if (index.equals("가입")) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btnLabel.equals("가입")) {
			if (cdb.userListInsert(this) > 0) {// 가입성공
				messageBox(this, name.getText() + "님 가입축드립니다.");
				dispose();// JDialog 창닫기

				cdb.userSelectAll(me.dt);// 모든레코드가져와서 DefaultTableModel에 올리기
			...
			if (id.getText().trim().equals("")) {
				...
				if (cdb.getIdByCheck(id.getText())) { // 중복아니다.(사용가능)
